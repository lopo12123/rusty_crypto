...
mod unit_test {
    ...
    fn encode_with_warmup() {
        fn run(i: u32) {
            ...
            let _result = V1Fix::encode_base64(&s);
    ...
    fn decode_with_warmup() {
        let str = V1Fix::encode_base64(&String::from("a".repeat(9000000)));

        let run = |i: u32| {
            let t_start = SystemTime::now();
            let result = V1Fix::decode_base64(&str);
    ...
    fn encode() {
        let str = "aaaaa";
        let result = V1Fix::encode_base64(&str);
